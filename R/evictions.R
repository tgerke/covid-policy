...
date_diff <- dat %>% 
  ...
  group_by(Party) %>% 
  summarize(mean_date = mean(`Froze evictions`)) %>% 
  mutate(
...
dat %>% 
  mutate(enacted = 
         !`Froze evictions` == lubridate::as_datetime(today() + 7)) %>%
  left_join(gov, by = "State") %>%
  arrange(Party, `Froze evictions`, State) %>% 
  mutate_at(vars(State), forcats::fct_inorder) %>% 
  ggplot() +  
  geom_point(
    aes(x = `Froze evictions`,
        y = State,
        color = Party,
        alpha = enacted),
    show.legend = FALSE) +
...
ggsave(here::here("plots", "evictions.png"), 
       device = "png",
       width = 8,
       height = 9,
       units = "in")
