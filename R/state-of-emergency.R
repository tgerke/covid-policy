...
date_diff <- dat %>% 
  ...
  group_by(Party) %>% 
  summarize(mean_date = mean(`State of emergency`)) %>% 
  mutate(
...
dat %>% 
  ...
  xlab(element_blank()) + 
  ylab(element_blank()) + 
  xlim(lubridate::as_datetime("2020-02-29"), lubridate::as_datetime(today())) + 
  geom_curve(
    data = tibble(
...
ggsave(here::here("plots", "state-of-emergency.png"), 
       device = "png",
       width = 8,
       height = 9,
       units = "in")
