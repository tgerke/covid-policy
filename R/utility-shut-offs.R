...
date_diff <- dat %>% 
  ...
  group_by(Party) %>% 
  summarize(mean_date = mean(`Order freezing utility shut offs`)) %>% 
  mutate(
...
dat %>% 
  mutate(enacted = 
         !`Order freezing utility shut offs` == lubridate::as_datetime(today() + 7)) %>%
  left_join(gov, by = "State") %>%
  arrange(Party, `Order freezing utility shut offs`, State) %>% 
  mutate_at(vars(State), forcats::fct_inorder) %>% 
  ggplot() +  
  geom_point(
    aes(x = `Order freezing utility shut offs`,
        y = State,
        color = Party,
        alpha = enacted),
    show.legend = FALSE) +
  ...
  geom_text(
    data = tibble(
      x = c(lubridate::as_datetime(today() - 1.5), 
            lubridate::as_datetime(today() + 4.4)),
      y = c(5.7, 23.5),
      Party = c("Democratic", "Republican"),
  ...
  labs(
    title = glue::glue(
      "Freeze on utility shut-offs from ",
...
ggsave(here::here("plots", "utility-shut-offs.png"), 
       device = "png",
       width = 8,
       height = 9,
       units = "in")
