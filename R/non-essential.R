...
date_diff <- dat %>% 
  ...
  group_by(Party) %>% 
  summarize(mean_date = mean(`Closed non-essential businesses`)) %>% 
  mutate(
...
dat %>% 
  mutate(enacted = 
         !`Closed non-essential businesses` == lubridate::as_datetime(today() + 7)) %>%
  left_join(gov, by = "State") %>%
  arrange(Party, `Closed non-essential businesses`, State) %>% 
  mutate_at(vars(State), forcats::fct_inorder) %>% 
  ggplot() +  
  geom_point(
    aes(x = `Closed non-essential businesses`,
        y = State,
        color = Party,
        alpha = enacted),
    show.legend = FALSE) +
  ...
  labs(
    title = glue::glue(
      "Non-essential business closures by ",
...
ggsave(here::here("plots", "non-essential.png"), 
       device = "png",
       width = 8,
       height = 9,
       units = "in")
